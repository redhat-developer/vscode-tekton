...
export function createVolumeClaim(event: Node & ParentNode, input: EditItem, newDivClass: string, initialValue: PipelineStart, pipelineRunVCT?: VCT): void {
  ...
  const selectSizeOp = new EditItem('', selectSize, 'option-workspace-id', input ? 'size-select-item' : (navigator.platform === 'Win32') ? 'size-select-item-vtc-window' : 'size-select-item-vtc', null, true);
